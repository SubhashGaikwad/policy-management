...
export class MemberService {
  ...
  protected convertDateFromClient(member: IMember): IMember {
    return Object.assign({}, member, {
      lastModified: member.lastModified?.isValid() ? member.lastModified.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((member: IMember) => {
        member.lastModified = member.lastModified ? dayjs(member.lastModified) : undefined;
      });
    }
    return res;
  }

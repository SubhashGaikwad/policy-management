...
export class VehicleClassComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.vehicleClassService
      ...
      .subscribe(
        (res: HttpResponse<IVehicleClass[]>) => {
  ...
  trackId(index: number, item: IVehicleClass): number {
    return item.id!;
  }

  delete(vehicleClass: IVehicleClass): void {
    const modalRef = this.modalService.open(VehicleClassDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.vehicleClass = vehicleClass;
  ...
  protected onSuccess(data: IVehicleClass[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/vehicle-class'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,

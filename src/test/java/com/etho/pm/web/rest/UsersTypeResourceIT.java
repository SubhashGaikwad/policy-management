...
class UsersTypeResourceIT {
    ...
    @Test
    @Transactional
    void getAllUsersTypesByUsersIsEqualToSomething() throws Exception {
        // Initialize the database
        usersTypeRepository.saveAndFlush(usersType);
        Users users;
        ...
        em.persist(users);
        em.flush();
        usersType.setUsers(users);
        users.setUsersType(usersType);
        usersTypeRepository.saveAndFlush(usersType);
        Long usersId = users.getId();

        // Get all the usersTypeList where users equals to usersId
        defaultUsersTypeShouldBeFound("usersId.equals=" + usersId);

        // Get all the usersTypeList where users equals to (usersId + 1)
        defaultUsersTypeShouldNotBeFound("usersId.equals=" + (usersId + 1));
    }

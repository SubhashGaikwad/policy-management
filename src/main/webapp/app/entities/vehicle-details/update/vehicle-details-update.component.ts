...
export class VehicleDetailsUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ vehicleDetails }) => {
      if (vehicleDetails.id === undefined) {
        ...
        vehicleDetails.registrationDate = today;
        vehicleDetails.lastModified = today;
      }
  ...
  protected updateForm(vehicleDetails: IVehicleDetails): void {
    this.editForm.patchValue({
      ...
      registrationDate: vehicleDetails.registrationDate ? vehicleDetails.registrationDate.format(DATE_TIME_FORMAT) : null,
      lastModified: vehicleDetails.lastModified ? vehicleDetails.lastModified.format(DATE_TIME_FORMAT) : null,
  ...
  protected createFromForm(): IVehicleDetails {
    return {
      ...
      registrationDate: this.editForm.get(['registrationDate'])!.value
        ? dayjs(this.editForm.get(['registrationDate'])!.value, DATE_TIME_FORMAT)
        : undefined,

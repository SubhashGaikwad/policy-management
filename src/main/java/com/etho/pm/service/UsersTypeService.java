...
public class UsersTypeService {
    ...
    /**
     *  Get all the usersTypes where Users is {@code null}.
     *  @return the list of entities.
     */
    @Transactional(readOnly = true)
    public List<UsersTypeDTO> findAllWhereUsersIsNull() {
        log.debug("Request to get all usersTypes where Users is null");
        return StreamSupport
            .stream(usersTypeRepository.findAll().spliterator(), false)
            .filter(usersType -> usersType.getUsers() == null)
            .map(usersTypeMapper::toDto)

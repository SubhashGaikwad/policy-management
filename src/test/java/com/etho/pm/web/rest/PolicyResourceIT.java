...
class PolicyResourceIT {
    ...
    private static final LocalDate DEFAULT_COMM_DATE = LocalDate.ofEpochDay(0L);
    private static final LocalDate UPDATED_COMM_DATE = LocalDate.now(ZoneId.systemDefault());
    private static final LocalDate SMALLER_COMM_DATE = LocalDate.ofEpochDay(-1L);
    ...
    private static final LocalDate SMALLER_POLICY_START_DATE = LocalDate.ofEpochDay(-1L);
    ...
    private static final LocalDate SMALLER_POLICY_END_DATE = LocalDate.ofEpochDay(-1L);
    ...
    private static final LocalDate DEFAULT_RISK_COVERED_FROM = LocalDate.ofEpochDay(0L);
    private static final LocalDate UPDATED_RISK_COVERED_FROM = LocalDate.now(ZoneId.systemDefault());
    private static final LocalDate SMALLER_RISK_COVERED_FROM = LocalDate.ofEpochDay(-1L);

    private static final LocalDate DEFAULT_RISK_COVERED_TO = LocalDate.ofEpochDay(0L);
    private static final LocalDate UPDATED_RISK_COVERED_TO = LocalDate.now(ZoneId.systemDefault());
    private static final LocalDate SMALLER_RISK_COVERED_TO = LocalDate.ofEpochDay(-1L);
    ...
    void getAllPolicies() throws Exception {
        ...
        restPolicyMockMvc
            ...
            .andExpect(jsonPath("$.[*].commDate").value(hasItem(DEFAULT_COMM_DATE.toString())))
            ...
            .andExpect(jsonPath("$.[*].policyEndDate").value(hasItem(DEFAULT_POLICY_END_DATE.toString())))
            ...
            .andExpect(jsonPath("$.[*].riskCoveredFrom").value(hasItem(DEFAULT_RISK_COVERED_FROM.toString())))
            .andExpect(jsonPath("$.[*].riskCoveredTo").value(hasItem(DEFAULT_RISK_COVERED_TO.toString())))
    ...
    void getPolicy() throws Exception {
        ...
        restPolicyMockMvc
            ...
            .andExpect(jsonPath("$.commDate").value(DEFAULT_COMM_DATE.toString()))
            ...
            .andExpect(jsonPath("$.policyEndDate").value(DEFAULT_POLICY_END_DATE.toString()))
            ...
            .andExpect(jsonPath("$.riskCoveredFrom").value(DEFAULT_RISK_COVERED_FROM.toString()))
            .andExpect(jsonPath("$.riskCoveredTo").value(DEFAULT_RISK_COVERED_TO.toString()))
    ...
    @Test
    @Transactional
    void getAllPoliciesByCommDateIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where commDate is greater than or equal to DEFAULT_COMM_DATE
        defaultPolicyShouldBeFound("commDate.greaterThanOrEqual=" + DEFAULT_COMM_DATE);

        // Get all the policyList where commDate is greater than or equal to UPDATED_COMM_DATE
        defaultPolicyShouldNotBeFound("commDate.greaterThanOrEqual=" + UPDATED_COMM_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByCommDateIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where commDate is less than or equal to DEFAULT_COMM_DATE
        defaultPolicyShouldBeFound("commDate.lessThanOrEqual=" + DEFAULT_COMM_DATE);

        // Get all the policyList where commDate is less than or equal to SMALLER_COMM_DATE
        defaultPolicyShouldNotBeFound("commDate.lessThanOrEqual=" + SMALLER_COMM_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByCommDateIsLessThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where commDate is less than DEFAULT_COMM_DATE
        defaultPolicyShouldNotBeFound("commDate.lessThan=" + DEFAULT_COMM_DATE);

        // Get all the policyList where commDate is less than UPDATED_COMM_DATE
        defaultPolicyShouldBeFound("commDate.lessThan=" + UPDATED_COMM_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByCommDateIsGreaterThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where commDate is greater than DEFAULT_COMM_DATE
        defaultPolicyShouldNotBeFound("commDate.greaterThan=" + DEFAULT_COMM_DATE);

        // Get all the policyList where commDate is greater than SMALLER_COMM_DATE
        defaultPolicyShouldBeFound("commDate.greaterThan=" + SMALLER_COMM_DATE);
    }
    ...
    @Test
    @Transactional
    void getAllPoliciesByPolicyStartDateIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where policyStartDate is greater than or equal to DEFAULT_POLICY_START_DATE
        defaultPolicyShouldBeFound("policyStartDate.greaterThanOrEqual=" + DEFAULT_POLICY_START_DATE);

        // Get all the policyList where policyStartDate is greater than or equal to UPDATED_POLICY_START_DATE
        defaultPolicyShouldNotBeFound("policyStartDate.greaterThanOrEqual=" + UPDATED_POLICY_START_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByPolicyStartDateIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where policyStartDate is less than or equal to DEFAULT_POLICY_START_DATE
        defaultPolicyShouldBeFound("policyStartDate.lessThanOrEqual=" + DEFAULT_POLICY_START_DATE);

        // Get all the policyList where policyStartDate is less than or equal to SMALLER_POLICY_START_DATE
        defaultPolicyShouldNotBeFound("policyStartDate.lessThanOrEqual=" + SMALLER_POLICY_START_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByPolicyStartDateIsLessThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where policyStartDate is less than DEFAULT_POLICY_START_DATE
        defaultPolicyShouldNotBeFound("policyStartDate.lessThan=" + DEFAULT_POLICY_START_DATE);

        // Get all the policyList where policyStartDate is less than UPDATED_POLICY_START_DATE
        defaultPolicyShouldBeFound("policyStartDate.lessThan=" + UPDATED_POLICY_START_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByPolicyStartDateIsGreaterThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where policyStartDate is greater than DEFAULT_POLICY_START_DATE
        defaultPolicyShouldNotBeFound("policyStartDate.greaterThan=" + DEFAULT_POLICY_START_DATE);

        // Get all the policyList where policyStartDate is greater than SMALLER_POLICY_START_DATE
        defaultPolicyShouldBeFound("policyStartDate.greaterThan=" + SMALLER_POLICY_START_DATE);
    }
    ...
    @Test
    @Transactional
    void getAllPoliciesByPolicyEndDateIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where policyEndDate is greater than or equal to DEFAULT_POLICY_END_DATE
        defaultPolicyShouldBeFound("policyEndDate.greaterThanOrEqual=" + DEFAULT_POLICY_END_DATE);

        // Get all the policyList where policyEndDate is greater than or equal to UPDATED_POLICY_END_DATE
        defaultPolicyShouldNotBeFound("policyEndDate.greaterThanOrEqual=" + UPDATED_POLICY_END_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByPolicyEndDateIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where policyEndDate is less than or equal to DEFAULT_POLICY_END_DATE
        defaultPolicyShouldBeFound("policyEndDate.lessThanOrEqual=" + DEFAULT_POLICY_END_DATE);

        // Get all the policyList where policyEndDate is less than or equal to SMALLER_POLICY_END_DATE
        defaultPolicyShouldNotBeFound("policyEndDate.lessThanOrEqual=" + SMALLER_POLICY_END_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByPolicyEndDateIsLessThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where policyEndDate is less than DEFAULT_POLICY_END_DATE
        defaultPolicyShouldNotBeFound("policyEndDate.lessThan=" + DEFAULT_POLICY_END_DATE);

        // Get all the policyList where policyEndDate is less than UPDATED_POLICY_END_DATE
        defaultPolicyShouldBeFound("policyEndDate.lessThan=" + UPDATED_POLICY_END_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByPolicyEndDateIsGreaterThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where policyEndDate is greater than DEFAULT_POLICY_END_DATE
        defaultPolicyShouldNotBeFound("policyEndDate.greaterThan=" + DEFAULT_POLICY_END_DATE);

        // Get all the policyList where policyEndDate is greater than SMALLER_POLICY_END_DATE
        defaultPolicyShouldBeFound("policyEndDate.greaterThan=" + SMALLER_POLICY_END_DATE);
    }
    ...
    @Test
    @Transactional
    void getAllPoliciesByPaymentDateIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where paymentDate is greater than or equal to DEFAULT_PAYMENT_DATE
        defaultPolicyShouldBeFound("paymentDate.greaterThanOrEqual=" + DEFAULT_PAYMENT_DATE);

        // Get all the policyList where paymentDate is greater than or equal to UPDATED_PAYMENT_DATE
        defaultPolicyShouldNotBeFound("paymentDate.greaterThanOrEqual=" + UPDATED_PAYMENT_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByPaymentDateIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where paymentDate is less than or equal to DEFAULT_PAYMENT_DATE
        defaultPolicyShouldBeFound("paymentDate.lessThanOrEqual=" + DEFAULT_PAYMENT_DATE);

        // Get all the policyList where paymentDate is less than or equal to SMALLER_PAYMENT_DATE
        defaultPolicyShouldNotBeFound("paymentDate.lessThanOrEqual=" + SMALLER_PAYMENT_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByPaymentDateIsLessThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where paymentDate is less than DEFAULT_PAYMENT_DATE
        defaultPolicyShouldNotBeFound("paymentDate.lessThan=" + DEFAULT_PAYMENT_DATE);

        // Get all the policyList where paymentDate is less than UPDATED_PAYMENT_DATE
        defaultPolicyShouldBeFound("paymentDate.lessThan=" + UPDATED_PAYMENT_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByPaymentDateIsGreaterThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where paymentDate is greater than DEFAULT_PAYMENT_DATE
        defaultPolicyShouldNotBeFound("paymentDate.greaterThan=" + DEFAULT_PAYMENT_DATE);

        // Get all the policyList where paymentDate is greater than SMALLER_PAYMENT_DATE
        defaultPolicyShouldBeFound("paymentDate.greaterThan=" + SMALLER_PAYMENT_DATE);
    }
    ...
    @Test
    @Transactional
    void getAllPoliciesByRiskCoveredFromIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where riskCoveredFrom is greater than or equal to DEFAULT_RISK_COVERED_FROM
        defaultPolicyShouldBeFound("riskCoveredFrom.greaterThanOrEqual=" + DEFAULT_RISK_COVERED_FROM);

        // Get all the policyList where riskCoveredFrom is greater than or equal to UPDATED_RISK_COVERED_FROM
        defaultPolicyShouldNotBeFound("riskCoveredFrom.greaterThanOrEqual=" + UPDATED_RISK_COVERED_FROM);
    }

    @Test
    @Transactional
    void getAllPoliciesByRiskCoveredFromIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where riskCoveredFrom is less than or equal to DEFAULT_RISK_COVERED_FROM
        defaultPolicyShouldBeFound("riskCoveredFrom.lessThanOrEqual=" + DEFAULT_RISK_COVERED_FROM);

        // Get all the policyList where riskCoveredFrom is less than or equal to SMALLER_RISK_COVERED_FROM
        defaultPolicyShouldNotBeFound("riskCoveredFrom.lessThanOrEqual=" + SMALLER_RISK_COVERED_FROM);
    }

    @Test
    @Transactional
    void getAllPoliciesByRiskCoveredFromIsLessThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where riskCoveredFrom is less than DEFAULT_RISK_COVERED_FROM
        defaultPolicyShouldNotBeFound("riskCoveredFrom.lessThan=" + DEFAULT_RISK_COVERED_FROM);

        // Get all the policyList where riskCoveredFrom is less than UPDATED_RISK_COVERED_FROM
        defaultPolicyShouldBeFound("riskCoveredFrom.lessThan=" + UPDATED_RISK_COVERED_FROM);
    }

    @Test
    @Transactional
    void getAllPoliciesByRiskCoveredFromIsGreaterThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where riskCoveredFrom is greater than DEFAULT_RISK_COVERED_FROM
        defaultPolicyShouldNotBeFound("riskCoveredFrom.greaterThan=" + DEFAULT_RISK_COVERED_FROM);

        // Get all the policyList where riskCoveredFrom is greater than SMALLER_RISK_COVERED_FROM
        defaultPolicyShouldBeFound("riskCoveredFrom.greaterThan=" + SMALLER_RISK_COVERED_FROM);
    }
    ...
    @Test
    @Transactional
    void getAllPoliciesByRiskCoveredToIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where riskCoveredTo is greater than or equal to DEFAULT_RISK_COVERED_TO
        defaultPolicyShouldBeFound("riskCoveredTo.greaterThanOrEqual=" + DEFAULT_RISK_COVERED_TO);

        // Get all the policyList where riskCoveredTo is greater than or equal to UPDATED_RISK_COVERED_TO
        defaultPolicyShouldNotBeFound("riskCoveredTo.greaterThanOrEqual=" + UPDATED_RISK_COVERED_TO);
    }

    @Test
    @Transactional
    void getAllPoliciesByRiskCoveredToIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where riskCoveredTo is less than or equal to DEFAULT_RISK_COVERED_TO
        defaultPolicyShouldBeFound("riskCoveredTo.lessThanOrEqual=" + DEFAULT_RISK_COVERED_TO);

        // Get all the policyList where riskCoveredTo is less than or equal to SMALLER_RISK_COVERED_TO
        defaultPolicyShouldNotBeFound("riskCoveredTo.lessThanOrEqual=" + SMALLER_RISK_COVERED_TO);
    }

    @Test
    @Transactional
    void getAllPoliciesByRiskCoveredToIsLessThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where riskCoveredTo is less than DEFAULT_RISK_COVERED_TO
        defaultPolicyShouldNotBeFound("riskCoveredTo.lessThan=" + DEFAULT_RISK_COVERED_TO);

        // Get all the policyList where riskCoveredTo is less than UPDATED_RISK_COVERED_TO
        defaultPolicyShouldBeFound("riskCoveredTo.lessThan=" + UPDATED_RISK_COVERED_TO);
    }

    @Test
    @Transactional
    void getAllPoliciesByRiskCoveredToIsGreaterThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where riskCoveredTo is greater than DEFAULT_RISK_COVERED_TO
        defaultPolicyShouldNotBeFound("riskCoveredTo.greaterThan=" + DEFAULT_RISK_COVERED_TO);

        // Get all the policyList where riskCoveredTo is greater than SMALLER_RISK_COVERED_TO
        defaultPolicyShouldBeFound("riskCoveredTo.greaterThan=" + SMALLER_RISK_COVERED_TO);
    }
    ...
    @Test
    @Transactional
    void getAllPoliciesByMaturityDateIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where maturityDate is greater than or equal to DEFAULT_MATURITY_DATE
        defaultPolicyShouldBeFound("maturityDate.greaterThanOrEqual=" + DEFAULT_MATURITY_DATE);

        // Get all the policyList where maturityDate is greater than or equal to UPDATED_MATURITY_DATE
        defaultPolicyShouldNotBeFound("maturityDate.greaterThanOrEqual=" + UPDATED_MATURITY_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByMaturityDateIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where maturityDate is less than or equal to DEFAULT_MATURITY_DATE
        defaultPolicyShouldBeFound("maturityDate.lessThanOrEqual=" + DEFAULT_MATURITY_DATE);

        // Get all the policyList where maturityDate is less than or equal to SMALLER_MATURITY_DATE
        defaultPolicyShouldNotBeFound("maturityDate.lessThanOrEqual=" + SMALLER_MATURITY_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByMaturityDateIsLessThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where maturityDate is less than DEFAULT_MATURITY_DATE
        defaultPolicyShouldNotBeFound("maturityDate.lessThan=" + DEFAULT_MATURITY_DATE);

        // Get all the policyList where maturityDate is less than UPDATED_MATURITY_DATE
        defaultPolicyShouldBeFound("maturityDate.lessThan=" + UPDATED_MATURITY_DATE);
    }

    @Test
    @Transactional
    void getAllPoliciesByMaturityDateIsGreaterThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where maturityDate is greater than DEFAULT_MATURITY_DATE
        defaultPolicyShouldNotBeFound("maturityDate.greaterThan=" + DEFAULT_MATURITY_DATE);

        // Get all the policyList where maturityDate is greater than SMALLER_MATURITY_DATE
        defaultPolicyShouldBeFound("maturityDate.greaterThan=" + SMALLER_MATURITY_DATE);
    }
    ...
    @Test
    @Transactional
    void getAllPoliciesByLastModifiedIsGreaterThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where lastModified is greater than or equal to DEFAULT_LAST_MODIFIED
        defaultPolicyShouldBeFound("lastModified.greaterThanOrEqual=" + DEFAULT_LAST_MODIFIED);

        // Get all the policyList where lastModified is greater than or equal to UPDATED_LAST_MODIFIED
        defaultPolicyShouldNotBeFound("lastModified.greaterThanOrEqual=" + UPDATED_LAST_MODIFIED);
    }

    @Test
    @Transactional
    void getAllPoliciesByLastModifiedIsLessThanOrEqualToSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where lastModified is less than or equal to DEFAULT_LAST_MODIFIED
        defaultPolicyShouldBeFound("lastModified.lessThanOrEqual=" + DEFAULT_LAST_MODIFIED);

        // Get all the policyList where lastModified is less than or equal to SMALLER_LAST_MODIFIED
        defaultPolicyShouldNotBeFound("lastModified.lessThanOrEqual=" + SMALLER_LAST_MODIFIED);
    }

    @Test
    @Transactional
    void getAllPoliciesByLastModifiedIsLessThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where lastModified is less than DEFAULT_LAST_MODIFIED
        defaultPolicyShouldNotBeFound("lastModified.lessThan=" + DEFAULT_LAST_MODIFIED);

        // Get all the policyList where lastModified is less than UPDATED_LAST_MODIFIED
        defaultPolicyShouldBeFound("lastModified.lessThan=" + UPDATED_LAST_MODIFIED);
    }

    @Test
    @Transactional
    void getAllPoliciesByLastModifiedIsGreaterThanSomething() throws Exception {
        // Initialize the database
        policyRepository.saveAndFlush(policy);

        // Get all the policyList where lastModified is greater than DEFAULT_LAST_MODIFIED
        defaultPolicyShouldNotBeFound("lastModified.greaterThan=" + DEFAULT_LAST_MODIFIED);

        // Get all the policyList where lastModified is greater than SMALLER_LAST_MODIFIED
        defaultPolicyShouldBeFound("lastModified.greaterThan=" + SMALLER_LAST_MODIFIED);
    }
